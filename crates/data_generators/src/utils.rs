...
const FEATHER_FILES_PATH: &str = "feather-data";
...
pub fn load_feather_json<T>(name: &str) -> Result<T, serde_json::Error>
where
    T: DeserializeOwned,
{
    serde_json::from_slice(&std::fs::read(format!("{}/{}", FEATHER_FILES_PATH, name)).unwrap())
}
...
pub fn output(file: &str, content: &str) {
    ...
    log::info!("Generated {}", path.to_str().unwrap());
...
fn rustfmt(path: impl Into<PathBuf> + Clone) {
    std::process::Command::new("rustfmt")
        .arg("+nightly") // formatting generated files is unstable
...
macro_rules! generate_enum_property {
    ...
    ($enum_name: ident, $property_name: literal, $typ: ty, $mapping: expr, $reverse: expr, $return_type: ty, $needs_bindings: expr $(,)?) => {{
        ...
        fns.push(quote! {
            #[doc = #doc]
            #[inline]
            pub const fn #property_name_ident(&self) -> $return_type {
                match self {
                    #(#self_to_prop),*
                }
            }
