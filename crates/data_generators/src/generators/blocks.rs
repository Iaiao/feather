...
pub fn generate() {
    ...
    for (name, dig_multipliers) in material_dig_multipliers {
        let multipliers = dig_multipliers
            ...
            .map(|(item, multiplier)| {
                ...
                quote! { (crate::items::ItemKind::#item_name, #multiplier) }
            })
            .collect::<Vec<_>>();
        ...
        dig_multiplier_constants.push(quote! {
            pub const #constant: &[(crate::items::ItemKind, f32)] = &[#(#multipliers),*];
        });
    ...
    out.extend(generate_enum_property!(
        BlockKind,
        "dig_multipliers",
        &'static [(crate::items::ItemKind, f32)],
        blocks
            .iter()
            .map(|block| {
                (
    ...
    out.extend(generate_enum_property!(
        BlockKind,
        "harvest_tools",
        Option<&'static [crate::items::ItemKind]>,
        blocks
            .iter()
            .map(|block| {
                (
                    ...
                    if let Some(harvest_tools) = block.harvest_tools.as_ref() {
                        let items = harvest_tools
                            ...
                            .collect::<Vec<_>>();
                        quote! {
                            Some(&[#(crate::items::ItemKind::#items),*])
                        }
                    } else {
                        quote! { None }
                    },
    ...
    output("crates/api/src/blocks/block.rs", out.to_string().as_str());
}
