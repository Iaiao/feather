...
use crate::common::chunk::worker::{
    ChunkLoadResult, LoadRequest, LoadedChunk, SaveRequest, WorkerRequest,
};
use crate::BiomeList;
...
impl RegionWorker {
    ...
    fn load_chunk(&mut self, req: LoadRequest) {
        ...
        if let ChunkLoadResult::Error(err) = &result {
            log::warn!("Couldn't load chunk at {}: {}", pos, err);
        }
    ...
    fn get_chunk_load_result(&mut self, req: LoadRequest) -> ChunkLoadResult {
        ...
        let chunk = match file.handle.load_chunk(pos, &*biomes) {
            Ok((chunk, _entity_data, _block_entities)) => chunk,
            Err(e) => match e {
    ...
    fn region_file_handle(&mut self, region: RegionPosition) -> Option<&mut OpenRegionFile> {
        match self.region_files.entry(region) {
            ...
            Entry::Vacant(e) => {
                let handle = anvil::region::load_region(&self.world_dir, region, self.world_height);
