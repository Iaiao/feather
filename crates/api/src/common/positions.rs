...
use crate::base::chunk::SECTION_WIDTH;
use crate::core::CHUNK_WIDTH;
...
#[derive(Copy, Clone, Debug, PartialEq, Serialize, Deserialize, Component)]
pub struct Position {
    pub x: f64,
    pub y: f64,
    pub z: f64,
...
impl<T: Into<f64>> Add<Vec3<T>> for Position {
...
impl ChunkPosition {
    ...
    pub fn first_block(self) -> BlockPosition {
        BlockPosition::new(
            self.x * SECTION_WIDTH as i32,
            0,
            self.z * SECTION_WIDTH as i32,
        )
    }
}
...
impl BlockPosition {
    ...
    pub const fn is_network_valid(self) -> bool {
        (-0x2000000 <= self.x && self.x < 0x2000000)
            && (-0x800 <= self.y && self.y < 0x800)
            && (-0x2000000 <= self.z && self.z < 0x2000000)
    }
}
