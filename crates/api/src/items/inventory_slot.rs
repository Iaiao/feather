...
impl InventorySlot {
        ...
        match self {
            Self::Filled(stack) => {
                if stack.count().get() <= amount {
                    ...
                    stack.set_count(stack.count().get() - amount).unwrap();
                    Self::Filled(out)
                }
            }
    ...
    pub fn count(&self) -> u32 {
        match self {
            InventorySlot::Filled(stack) => stack.count().get(),
...
#[cfg(test)]
mod test {
    use crate::{InventorySlot, ItemKind};

    #[test]
    fn test_merge() {
        let a = &mut InventorySlot::new(ItemKind::Stone, 5);
        let b = &mut InventorySlot::new(ItemKind::Stone, 5);
        a.merge(b);
        println!("{:?}", a);
    ...
    fn transfer_to() {
        let a = &mut InventorySlot::new(ItemKind::Stone, 5);
        let b = &mut InventorySlot::new(ItemKind::Stone, 2);
        a.transfer_to(2, b);
        assert_eq!(a.count(), 3);
        assert_eq!(b.count(), 4);
