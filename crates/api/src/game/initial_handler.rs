...
use crate::game::{connection_worker::Worker, favicon::Favicon};
use crate::protocol::{
    codec::CryptKey,
    packets::{
...
use crate::components::ProfileProperty;
...
pub async fn handle(worker: &mut Worker) -> anyhow::Result<InitialHandling> {
    ...
    match handshake.next_state {
        ...
        HandshakeState::Login => {
            ...
            let proxy_data = if let Some(crate::game::options::ProxyMode::Bungeecord) =
                worker.options().proxy_mode
            {
...
async fn handle_login(
    ...
    if let Some(crate::game::options::ProxyMode::Velocity) = worker.options().proxy_mode {
