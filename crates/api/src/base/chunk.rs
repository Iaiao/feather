...
impl Chunk {
    ...
    pub fn section(&self, y: usize) -> Option<&ChunkSection> {
        self.sections.get(y + 1)
    }

    /// Mutably gets the chunk section at index `y`.
    pub fn section_mut(&mut self, y: usize) -> Option<&mut ChunkSection> {
        self.sections.get_mut(y + 1)
    }

    /// Sets the section at index `y`.
    pub fn set_section_at(&mut self, y: usize, section: ChunkSection) {
        self.sections[y + 1] = section;
    }
...
mod tests {
    ...
    fn section_from_data_and_palette() {
        ...
        let mut data = PackedArray::new(
            16 * SECTION_WIDTH * SECTION_WIDTH,
