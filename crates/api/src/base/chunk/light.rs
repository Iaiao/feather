use crate::base::chunk::ChunkSection;
...
impl LightStore {
    ...
    pub fn new() -> Self {
        let mut this = LightStore {
            sky_light: Some(PackedArray::new(
                SECTION_VOLUME,
                NonZeroUsize::new(4).unwrap(),
            )),
            block_light: Some(PackedArray::new(
                SECTION_VOLUME,
                NonZeroUsize::new(4).unwrap(),
            )),
        };
    ...
    pub fn from_packed_arrays(
        ...
        if (sky_light.is_some()
            && (sky_light.as_ref().unwrap().len() != SECTION_VOLUME
                || sky_light.as_ref().unwrap().bits_per_value().get() != 4))
            || (block_light.is_some()
                && (block_light.as_ref().unwrap().len() != SECTION_VOLUME
                    || block_light.as_ref().unwrap().bits_per_value().get() != 4))
        {
            None
        } else {
            Some(Self {
