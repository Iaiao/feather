...
use crate::base::chunk::{PackedArray, BIOMES_PER_CHUNK_SECTION, SECTION_VOLUME};
use crate::{BiomeId, BlockId, ChunkSection};
    ...
    T: Paletteable,
{
    ...
    pub fn palette_bits_per_value(palette_len: NonZeroUsize) -> NonZeroUsize {
        NonZeroUsize::new((palette_len.get() as f64).log2().ceil() as usize).unwrap()
    }
        ...
        match self {
            ...
            PalettedContainer::MultipleValues { data, palette } => {
                if palette.len() >= data.max_value() as usize {
                    ...
                    let new_size = NonZeroUsize::new(data.bits_per_value().get() + 1).unwrap();
...
impl Paletteable for BlockId {
    ...
    fn length() -> NonZeroUsize {
        NonZeroUsize::new(HIGHEST_ID).unwrap()
    }
}
...
impl Paletteable for BiomeId {
    ...
    fn length() -> NonZeroUsize {
        NonZeroUsize::new(BIOMES_COUNT.load(Ordering::Relaxed)).expect("No biomes are initialized")
    }
}
