...
use feather_consts::{ANVIL_VERSION, VERSION_STRING};
...
use crate::base::chunk::{LightStore, PackedArray};
use crate::base::world::WorldHeight;
use crate::common::positions::ChunkPosition;
use crate::{BiomeId, BiomeList};
...
fn read_section_into_chunk(
    ...
    assert!(section.y as i32 >= min_y_section);
    chunk.set_section_at(
        (section.y as isize - min_y_section as isize) as usize,
        chunk_section,
    );
...
fn chunk_to_data_chunk(
    ...
    let sections = chunk.sections().len() - 2;
    DataChunk {
                ...
                LevelSection {
                    ...
                    block_states: match section.blocks() {
                        ...
                        PalettedContainer::GlobalPalette { data } => {
                            // Convert to a MultipleValues palette
                            ...
                            let data =
                                data.resized(PalettedContainer::<BlockId>::palette_bits_per_value(
                    ...
                    biomes: match section.biomes() {
                        ...
                        PalettedContainer::GlobalPalette { data } => {
                            ...
                            let data =
                                data.resized(PalettedContainer::<BiomeId>::palette_bits_per_value(
                                    NonZeroUsize::new(palette.len()).unwrap(),
                                ));
                            PaletteAndData {
                                palette: palette
                                    .iter()
...
impl Display for Error {
    fn fmt(&self, f: &mut Formatter) -> Result<(), fmt::Error> {
        match self {
            ...
            Error::UnsupportedDataVersion(_) => write!(f, "The chunk uses an unsupported data version. Feather currently only supports {} region files.", VERSION_STRING)?,
