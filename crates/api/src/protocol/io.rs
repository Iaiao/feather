...
use crate::items::{ItemKind, ItemStackBuilder};

use crate::base::block::NetworkBlockPosition;
use crate::base::chunk::paletted_container::{Paletteable, PalettedContainer};
use crate::base::metadata::EntityMetadata;
use crate::base::metadata::MetaEntry;
...
impl Readable for Slot {
        ...
        Self: Sized,
    {
        ...
        if present {
            ...
            let tags: Option<HashMap<String, nbt::Value>> =
                Nbt::read(buffer, version).ok().map(|nbt| nbt.0);
            if tags.is_none() {
                buffer.set_position(position + 1); // account for TAG_End, which is 1 byte
            }

            let item = ItemKind::from_id(item_id.try_into()?)
                .ok_or_else(|| anyhow!("unknown item ID {}", item_id))?;

            // Todo fix: Panics if count is zero
            let mut item = ItemStackBuilder::new(item).count(count).build();
...
impl Writeable for Slot {
    fn write(&self, buffer: &mut Vec<u8>, version: ProtocolVersion) -> anyhow::Result<()> {
        ...
        if let Filled(stack) = self {
            ...
            (stack.count().get() as u8).write(buffer, version)?;
...
fn read_meta_entry(
    ...
    Ok(match id {
        ...
        9 => MetaEntry::Position(NetworkBlockPosition::read(buffer, version)?.into()),
        10 => MetaEntry::OptPosition(if bool::read(buffer, version)? {
            Some(NetworkBlockPosition::read(buffer, version)?.into())
        } else {
            None
        }),
...
fn write_meta_entry(
    ...
    match entry {
        ...
        MetaEntry::Position(x) => NetworkBlockPosition::try_from(x)
            .expect("Invalid BlockPosition in MetaEntry::Position")
            .write(buffer, version)?,
        MetaEntry::OptPosition(ox) => {
            if let Some(x) = ox {
                true.write(buffer, version)?;
                NetworkBlockPosition::try_from(x)
                    .expect("Invalid BlockPosition in MetaEntry::OptPosition")
                    .write(buffer, version)?;
            } else {
                false.write(buffer, version)?;
            }
...
impl Readable for NetworkBlockPosition {
    fn read(buffer: &mut Cursor<&[u8]>, version: ProtocolVersion) -> anyhow::Result<Self>
    where
        Self: Sized,
    {
...
impl Writeable for NetworkBlockPosition {
    fn write(&self, buffer: &mut Vec<u8>, version: ProtocolVersion) -> anyhow::Result<()> {
